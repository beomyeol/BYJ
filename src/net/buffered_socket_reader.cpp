#include "byj/net/buffered_socket_reader.h"
#include <cstring>
#include <sys/types.h>
...
int BufferedSocketReader::read(char* buffer, std::size_t buffer_size, std::size_t offset) {
    std::size_t i;
...
int BufferedSocketReader::skip(std::size_t size) {
    ...
    memmove(buffer_, buffer_ + actual_skipped_size, buffer_size_ - actual_skipped_size);
